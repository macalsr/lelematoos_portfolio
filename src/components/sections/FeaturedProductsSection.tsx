...
export function FeaturedProductsSection({ products, categories, whatsappPhone }: FeaturedProductsSectionProps) {
  ...
  const featuredProducts = useMemo(
    () => products.filter((product) => product.destaque && product.disponivel),
    [products],
  );

  const visibleProducts = useMemo(() => {
    if (activeCategory === "todas") return featuredProducts;
    return featuredProducts.filter((product) => product.categoria === activeCategory);
      ...
      <Container>
        ...
        <div className="mt-6 flex flex-wrap gap-2 max-md:mt-5">
          <button
            ...
            className={`rounded-full border-[4px] border-green-dark px-3 py-2 text-[11px] font-black uppercase tracking-[0.12em] ${
              activeCategory === "todas" ? "bg-green-dark text-white" : "bg-surface text-green-dark"
            }`}
          >
            Todas
          </button>
          ...
          {categories.map((category) => (
            <button
              ...
              className={`rounded-full border-[4px] border-green-dark px-3 py-2 text-[11px] font-black uppercase tracking-[0.12em] ${
                activeCategory === category.id ? "bg-green-dark text-white" : "bg-surface text-green-dark"
              }`}
            >
              {category.nome}
            </button>
          ...
          {visibleProducts.map((product) => (
            <Card key={product.id} variant="flash">
              <Link
                ...
                className="flex aspect-square items-center justify-center border-b-[4px] border-b-green-dark bg-[linear-gradient(135deg,#f6d5df,#f8e6ec_42%,#dcebcf)] p-[22px]"
              >
                <img
